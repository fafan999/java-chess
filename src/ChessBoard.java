...
//			BufferedImage all = ImageIO.read(new File("chess.png")); 			
			...
			for (int y=0;y<400;y+=200) {
				for (int x = 0;x<1200;x+=200) {
					pieceImages[index] = all.getSubimage(x, y, 200, 200).getScaledInstance(SIZE_OF_SQUARE, SIZE_OF_SQUARE, BufferedImage.SCALE_SMOOTH);
					index++;
				}
			}			
		} catch (Exception e) {
...
//					System.out.println(selectedPiece.coordinate.toString());
	...
	public void paint(Graphics g) {
		...
		for (Point p: legalMoves()) {
			if (getPiece(p) == null) {
				g.fillOval(p.x*SIZE_OF_SQUARE+SIZE_OF_SQUARE/3, (7-p.y)*SIZE_OF_SQUARE+SIZE_OF_SQUARE/3, SIZE_OF_SQUARE/3, SIZE_OF_SQUARE/3);
			} else {
				...
				int[] b1 = {(7-p.y)*SIZE_OF_SQUARE, (7-p.y)*SIZE_OF_SQUARE+SIZE_OF_SQUARE/4, (7-p.y)*SIZE_OF_SQUARE};
				g.fillPolygon(a1, b1, 3);		
				// top right
		        int[] a2 = {(p.x+1)*SIZE_OF_SQUARE-SIZE_OF_SQUARE/4, (p.x+1)*SIZE_OF_SQUARE, (p.x+1)*SIZE_OF_SQUARE};
		        int[] b2 = {(7-p.y)*SIZE_OF_SQUARE, (7-p.y)*SIZE_OF_SQUARE, (7-p.y)*SIZE_OF_SQUARE+SIZE_OF_SQUARE/4};
		        g.fillPolygon(a2, b2, 3);
		        // bottom right
		        int[] a3 = {(p.x+1)*SIZE_OF_SQUARE-SIZE_OF_SQUARE/4, (p.x+1)*SIZE_OF_SQUARE, (p.x+1)*SIZE_OF_SQUARE};
		        int[] b3 = {(7-p.y+1)*SIZE_OF_SQUARE, (7-p.y+1)*SIZE_OF_SQUARE, (7-p.y+1)*SIZE_OF_SQUARE-SIZE_OF_SQUARE/4};
		        g.fillPolygon(a3, b3, 3);
		        // bottom left
		        int[] a4 = {p.x*SIZE_OF_SQUARE, p.x*SIZE_OF_SQUARE, p.x*SIZE_OF_SQUARE+SIZE_OF_SQUARE/4};
		        int[] b4 = {(7-p.y+1)*SIZE_OF_SQUARE, (7-p.y+1)*SIZE_OF_SQUARE-SIZE_OF_SQUARE/4, (7-p.y+1)*SIZE_OF_SQUARE};
